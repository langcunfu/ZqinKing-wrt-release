...
# ============================================================================
# 【U-Boot 策略：使用 vikingyfy 2026.07 原编译 U-Boot，不再替换】
# 实机验证结论（2026-09-23）：
#   - vikingyfy 2026.07：能看到串口跑码（U-Boot 正常运行），卡在内核启动
#   - zhoufuli 2024.10（从原版固件提取）：完全无反应（SPL 阶段即失败）
# 问题已锁定在内核侧，故保留 2026.07 U-Boot，不再执行任何替换。
# 原版 2024.10 资产保留在 wrt_core/patches/nsy_g68/uboot_zhoufuli/ 备查。
# ============================================================================
...
echo "==> firmware 目录内容: $(ls -1 "$FIRMWARE_DIR" 2>/dev/null | tr '\n' ' ')"
